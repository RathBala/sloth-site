...
- **Purpose:** Public, lightweight developer page for the Sloth Agent API beta. It provides the real npm installation command and public CLI source, then explains how paid Sloth users can create a Developer access token, save it with the CLI's native login, inspect their cache-only account inventory, read categories and transactions, create partner clarification links, and apply category assignments.
