...
- **Pricing:** The same pricing block as [`src/index.html`](../src/index.html) (monthly/yearly toggle via [`pricing-toggle.js`](../src/assets/js/pricing-toggle.js)) appears after the FAQ and before the closing CTA. The primary button in that block uses `data-analytics-cta="wedding-pricing-start"`. Trial-card copy matches the homepage and spells out that **one subscription covers both partners** (two accounts, one price) from signup.
